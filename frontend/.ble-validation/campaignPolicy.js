export const CAMPAIGN_INTENT_OPTIONS = [
...
export const NEGATIVE_CONTROL_OPTIONS = [
...
export function evaluateCampaignPolicy(input) {
    if (input.intent === 'positive_target_validation') {
        if (!input.targetSelected)
            return { allowed: false, reason: 'Seleccione un objetivo específico para una validación positiva.' };
        if (!input.seenNow)
            return { allowed: false, reason: 'La validación positiva está bloqueada: el último escaneo no recibió advertising de ese objetivo. Recargar historial no realiza un escaneo; enciende o despierta el sensor y pulsa Buscar BLE ahora.' };
    }
    if (input.intent === 'negative_control') {
        if (!input.targetSelected)
            return { allowed: false, reason: 'Seleccione el objetivo al que se aplica el control negativo.' };
        if (!input.negativeControlType)
            return { allowed: false, reason: 'Declare la condición del control negativo antes de capturar.' };
        if (!input.operatorConfirmation)
            return { allowed: false, reason: 'Confirme que la condición negativa se verificó físicamente antes de capturar.' };
    }
...
export function campaignContract(intent, negativeControlType, operatorConfirmation) {
    return {
